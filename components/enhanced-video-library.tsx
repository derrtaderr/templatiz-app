...
import { Download, Share2, Edit, Search, Filter, Folder } from 'lucide-react'
...
export function EnhancedVideoLibrary() {
  ...
  const filterVideos = () => {
    ...
    if (engagementFilter) {
      filteredVideos = filteredVideos.filter(video => video.engagementLevel === engagementFilter)
    }

    if (selectedFolder) {
      filteredVideos = filteredVideos.filter(video => video.folder === selectedFolder)
    }
